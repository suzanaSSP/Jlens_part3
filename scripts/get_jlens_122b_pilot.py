...
GPUs another job is already using. Must happen before `import torch`.

Usage:
    python -m scripts.get_jlens_122b_pilot [--n 2] [--out-dir jspace_prompts_122b]
"""
...
def _select_gpus(min_free_gb: float, max_util_pct: float) -> list[int]:
    ...
    eligible = [r for r in rows if r[1] >= min_free_gb * 1024 and r[3] <= max_util_pct]
    ...
    for idx, free_mb, total_mb, util_pct in rows:
        tag = "SELECTED" if any(idx == e[0] for e in eligible) else "skipped (busy)"
        print(f"  gpu{idx}: {free_mb}/{total_mb} MiB free, {util_pct}% util  -> {tag}")

    if not eligible:
        print(
            f"No GPUs met the bar (>= {min_free_gb} GB free, <= {max_util_pct}% util). "
            "Refusing to guess -- pass --min-free-gb / --max-util-pct to relax."
        )
        sys.exit(1)
...
# cores, so transformers dequantizes to bf16 on load -- roughly doubling the
# ~119GB on-disk footprint to ~240GB, plus a transient per-layer buffer
# during the fp8->bf16 conversion itself. That leaves ~0 headroom across
# 8x32GB GPUs for activations/KV-cache, which is what OOM'd. Capping
# per-GPU usage and letting accelerate overflow the rest onto CPU RAM (see
# max_memory below) trades speed for actually completing.
...
def main():
    ...
    parser.add_argument(
        "--cpu-gib", type=float, default=200.0,
    ...
    max_memory["cpu"] = f"{args.cpu_gib}GiB"
